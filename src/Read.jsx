...
function Read() {
  ...
  useEffect(() => {
    if (!worker.current) {
      worker.current = new Worker(new URL('./workerreadalong.js', import.meta.url), {
        type: 'module'
      });
    }
    ...
    const onMessageReceived = (e) => {
      switch (e.data.status) {
        ...
        case 'start':
          ...
          break;
        case 'update':
          const { tps, partialText } = e.data;
          setTps(tps);
          const updatedText = text + partialText;
          setText(updatedText);
          highlightMatchingText(updatedText, userInput);
          break;
        case 'complete':
          setIsProcessing(false);
          setText(e.data.output);
          highlightMatchingText(e.data.output, userInput);
          break;
      }
    };
  ...
  // Text Matching
 const highlightMatchingText = (transcription, userInput) => {
  if (transcription && userInput) {
    const regExp = new RegExp(`(${userInput.split(' ').join('|')})`, 'gi');
    const highlighted = transcription.replace(regExp, (match) => `<span style="color: green;">${match}</span>`);
        ...
        {(
          <div className="h-full overflow-auto scrollbar-thin flex justify-center items-center flex-col relative">
            <div className="flex flex-col items-center mb-1 max-w-[400px] text-center">
              ...
              <h1 className="text-4xl font-bold mb-1">READ-ALONG</h1>
              <h2 className="text-xl font-semibold">Real-time Read along in-browser </h2>
            </div>
            ...
            <div className="flex flex-col items-center px-4">
              ...
              {status === 'ready' && (
                <div className="relative w-full flex flex-col items-center">
                  <textarea
                    className="w-full mt-4 p-2 border text-black rounded-lg"
                    placeholder="Enter text here..."
                    value={userInput}
                    onChange={(e) => {
                      setUserInput(e.target.value);
                      highlightMatchingText(text, e.target.value);
                    }}
                  />
